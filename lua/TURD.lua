Settings = {
Name = "*TURD (Moving Average)",
FastPeriod = 12,
SlowPeriod = 100,
Metod = "VMA", --(SMA, MMA, EMA, WMA, SMMA, VMA)
...
function Turd() --Turtle and Dophin
	local turd_Fast=MA()
	local turd_Slow=MA()
	local it = {p=0, l=0}
return function (I, Fsettings, ds)
local Fsettings=(Fsettings or {})
local FastPeriod = (Fsettings.FastPeriod or 12)
local SlowPeriod = (Fsettings.SlowPeriod or 100)
local M = (Fsettings.Metod or VMA)
local VT = (Fsettings.VType or CLOSE)
if (FastPeriod > 0) then
	if I == 1 then
		it = {p=0, l=0}
	end
	local b_fast = turd_Fast(I, FastPeriod,{Metod = M, VType=VT}, ds)
	local b_slow = turd_Slow(I, SlowPeriod, {Metod = M, VType=VT}, ds)
	if CandleExist(I,ds) then
		if I~=it.p then it={p=I, l=it.l+1} end
		if it.l >= SlowPeriod and b_fast and b_slow then
...
return function (I, P, Fsettings, ds)
	local Fsettings=(Fsettings or {})
	if (P > 0) then
